...
export default {
  ...
  methods:{
    ...
    async sendBanner(){
      this.loading = true
      if(this.banner.imageTM.length!==0){
        try {
          const res = await this.$axios.post(`/admin/banners/add`,{});
          if(res.status == 201){
            const ress = await this.$axios.post(`/admin/banners/upload-image/${res.data.banner_id}`,this.returnFormData())
            console.log(ress);
          }
