<template>
  <div class="textAdd">
    <v-row style="padding:12px 24px">
      <v-col cols="5">
        <v-text-field dense :label="$t('name')" :rules="rules" v-model="text.name"></v-text-field>
      </v-col>
      <v-col cols="1"></v-col>
      <v-col cols="5">
        <v-text-field  dense :label="$t('phoneNumber')" :rules="rules" v-model="text.number"></v-text-field>
      </v-col>
      <v-col cols="5">
        <v-text-field  dense :label="$t('password')" :rules="rules" v-model="text.password"></v-text-field>
      </v-col>
...
export default {
  ...
  methods:{
    async send(){
      this.loading = true
      if(this.text.name && this.text.number && this.text.password){
        try {
          const {status} = await this.$axios.post('/admin/seller/add',this.text)
          if(status === 201){
