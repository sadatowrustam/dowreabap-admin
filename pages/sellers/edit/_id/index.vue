...
export default {
  ...
  async mounted(){
    try {
      ...
      let { data } = await this.$axios.get(`/public/seller/${productId}`);
      this.text = data.seller
      console.log(data)
    } catch (err) {
      console.log(err)
    }
  ...
  methods:{
    async send(){
      this.loading = true
      if(this.text.body_tm && this.text.body_ru && this.text.name){
        try {
          const {status} = await this.$axios.patch('/admin/seller/'+this.$route.params.id,this.text)
          if(status === 200){
