<template>
  <div>
    ...
    <v-text-field :rules="rules" :label="$t('eng') +' '+ $t('name')" v-model="data.name_en"></v-text-field>
    <v-btn
      class="ma-2"
      :loading="loading"
      :disabled="loading"
...
export default {
  ...
  mounted(){
    ...
    this.data.name_en=category.name_en
    this.id = category.category_id
  },
  methods:{
    async sendCategory(){
      this.loading = true
      if(this.data.name_tm && this.data.name_ru && this.data.name_en){
        try {
